...
const agregarGasto = () => {
    ...
    if (cantidad <= 0) {
        error.value = 'La cantidad no válida'
        setTimeout(() => {
            error.value = ''
        }, 3000);
        return
    ...
    // Validar que el usuario no gaste mas de lo disponible, si es asi solicitar confirmación
    if (id) {
...
const solicitarConfirmacion = () => {
    Swal.fire({
        title: '¿Estás seguro?',
        text: 'El gasto que intentas agregar es mayor a lo disponible',
        icon: 'warning',
...
<template>
    <div class="modal">
        <div class="cerrar-modal">
            <img :src="cerrarModal" alt="boton cerrar modal" @click="emit('cerrar-modal')">
        </div>
